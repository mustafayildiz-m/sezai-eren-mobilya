...
<a href="/projeler/<?= e($p['slug']) ?>" class="card img-zoom group block reveal reveal-delay-<?= $delay % 4 ?>">
  <div class="relative aspect-[4/3] overflow-hidden rounded-2xl">
    <?php if ($p['cover']): ?>
      <img src="/uploads/<?= e($p['cover_thumb'] ?: $p['cover']) ?>" srcset="/uploads/<?= e($p['cover_thumb'] ?: $p['cover']) ?> 480w, /uploads/<?= e($p['cover']) ?> 1600w" sizes="(min-width:1024px) 33vw, (min-width:640px) 50vw, 100vw" alt="<?= e($p['title']) ?> – <?= e($p['category_name'] ?? '') ?> Ankara" width="480" height="360" loading="lazy" class="h-full w-full object-cover">
    <?php else: ?>
